...
export function Leases() {
  ...
  const fetchLeases = async () => {
    try {
      ...
      const response = await fetch(
        "https://assettone-rental-management.onrender.com/api/v1/leases/",
        {
          headers: {
  ...
  const handleDelete = async (id: string) => {
    try {
      const accessToken = localStorage.getItem("accessToken");
      const response = await fetch(
        `https://assettone-rental-management.onrender.com/api/v1/leases/${id}/`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  const handleLeaseTransfer = async (transferData: TransferLeaseData) => {
    try {
      const accessToken = localStorage.getItem("accessToken");
      const response = await fetch(
        `https://assettone-rental-management.onrender.com/api/v1/leases/${selectedLease?.id}/transfer_lease/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleTerminate = async (id: string) => {
    try {
      const accessToken = localStorage.getItem("accessToken");
      const response = await fetch(
        `https://assettone-rental-management.onrender.com/api/v1/leases/${id}/terminate_lease/`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
